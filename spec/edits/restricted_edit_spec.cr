  ...
  describe ".distance" do
    ...
    context "when max is 0" do
      it "returns 0 for identical strings" do
        Edits::RestrictedEdit.distance("abc", "abc", 0).should eq 0
      end

      it "returns 0 for different strings" do
        Edits::RestrictedEdit.distance("abc", "abd", 0).should eq 0
      end
    end

    context "when max is less than actual distance" do
      it "returns 2 for cloud, crayon" do
        Edits::RestrictedEdit.distance("cloud", "crayon", 2).should eq 2
      end

      it "returns 4 for cloud, crayon" do
        Edits::RestrictedEdit.distance("cloud", "crayon", 4).should eq 4
      end
    end
  end
